...
public class ReversiSpielTest {
    ...
    public void setUp() throws Exception {
        ...
        reversi1.initSpiel(8, views);// besetzt die mittleren Felder und ruft schon 1mal update f�r alle Views auf
    }

    private void registriereSpieler(ReversiSpiel reversi) {
        spieler1 = reversi.registriereSpieler("Mathi");  // ruft update f�r alle Views auf
        spieler2 = reversi.registriereSpieler("Robert"); // ruft update f�r alle Views auf
    }
    ...
    public final void testWoKann() {
        ...
        Set<Pos> setPosWoKann = new HashSet<>();
    ...
    public final void testSetzeSpielstein() {
        ...
        Set<Pos> woKannResult = new HashSet<>();
    ...
    public void testUndo() {
        ...
        Spielfeld mockSpielfeld = mock(Spielfeld.class);
        ReversiSpiel spyReversi = spy(reversi1);
        doReturn(mockSpielfeld).when(spyReversi).getSpielfeldFromPositionInList();
        doReturn(spieler1).when(spyReversi).spielerAmZug();
        doReturn(spieler2).when(spyReversi).getGegner(spieler1);
        
        spyReversi.undo();
        
        Assert.assertEquals(mockSpielfeld, spyReversi.getSpielfeld());
        Assert.assertTrue(spieler2.isAmZug());
    }
    @Test
    public void testRedo() {
        ...
        Spielfeld mockSpielfeld = mock(Spielfeld.class);
        ReversiSpiel spyReversi = spy(reversi1);
        spyReversi.setSpielfeld(mockSpielfeld);
        //doReturn(mockSpielfeld).when(spyReversi).getSpielfeldFromPositionInList();
        doReturn(spieler1).when(spyReversi).spielerAmZug();
        doReturn(spieler2).when(spyReversi).getGegner(spieler1);
    ...
    @Test
    public void testInvertSpielerAmZug() {
        registriereSpieler(reversi1);
        Spielfeld mockSpielfeld = mock(Spielfeld.class);
        ReversiSpiel spyReversi = spy(reversi1);
        doReturn(spieler1).when(spyReversi).spielerAmZug();
        ...
        spyReversi.invertSpielerAmZug();
        
        Assert.assertTrue(spieler2.isAmZug());
        Assert.assertFalse(spieler1.isAmZug());
    }
    ...
    public void testGetFeldZustandErsterSpielerAmZug() {
        ...
        Assert.assertEquals(SpielfeldFeldZustand.LEER_UND_NICHT_BESETZBAR, spyReversi.getFeldZustand(0, 1)); // !!!! Die Zust�nde h�ngen davon ab, wer dran ist!!!
    }
    ...
    public void testGetFeldZustandZweiterSpielerAmZug() {
        ...
        Assert.assertEquals(SpielfeldFeldZustand.LEER_UND_BESETZBAR2, spyReversi.getFeldZustand(0, 3)); // !!!! Die Zust�nde h�ngen davon ab, wer dran ist!!!
    }
